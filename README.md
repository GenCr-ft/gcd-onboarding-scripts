...
4.  **Installation:** Installs binaries (nvm, pyenv, OpenTofu, etc.) and verifies Docker/AWS CLI. `gft` itself is delegated to the cloned `gcs-plt-tools/onboard.sh` flow once the workspace exists, so there is one canonical owner for the global wrapper contract.
